...
public class OrderQueueTester {
    ...
    public void AddingNodeToEmptyTreeShouldCreateRoot() {
        ...
        Order order = new Order("SPY", 1, 1005, 500);
        oq.add(order);
    ...
    public void RandomTestingAgainstJavaUtilPriorityTesting() {
        ...
        for(int i=0; i < 500; i++) {
            ...
            Order order = new Order("SPY", 1, price, quantity);
        ...
        for(int i=0; i < 500; i++) {
            ...
            Assert.assertEquals(oqOrder.getPrice(), pqOrder.getPrice());
            Assert.assertEquals(oqOrder.getRemainingQuantity(), pqOrder.getRemainingQuantity());
        }
    }
}
